...
namespace sahhh
{
    public partial class Form1 : Form
    {
        ...
        // --- REZULTAT (Promenjeno u double zbog pola poena) ---
        ...
        private void BtnRemi_Click(object sender, EventArgs e)
        {
            ...
            {
                // POPRAVLJENO: Dodavanje po pola boda za remi i osvežavanje rezultata
                resultadoBeli += 0.5;
                rezultatCrni += 0.5;
                OsveziNaslovProzora();
        ...
        bool isPossible(int ppolje, int kpolje, string figura, bool trenutniPotez)
        {
            int pRed = ppolje / 8;
            int pKol = ppolje % 8;
            int kRed = kpolje / 8;
            ...
            else
            {
                ...
                if (validanPotez)
                {
                    ...
                    if (figura == "K" && Math.Abs(pKol - kKol) == 2)
                    {
                        ...
                        board[kRed, tCiljK].Text = boardf[kRed, tCiljK];
                        board[kRed, tIzvorK].Text = "";
                    }
                    ...
                    if (figura == "P" && (kRed == 0 || kRed == 7))
                    {
                        ...
                        promocijaForma.MaximizeBox = false;
                        promocijaForma.MinimizeBox = false;
                        ...
                        Button btnN = new Button() { Text = "Konj (N)", Location = new Point(265, 20), Size = new Size(75, 40) };
                        ...
                        promocijaForma.Controls.Add(btnQ);
                        promocijaForma.Controls.Add(btnR);
                        promocijaForma.Controls.Add(btnB);
                        promocijaForma.Controls.Add(btnN);
                    ...
                    if (figura == "K") { if (mojaBoja == "W") beliKraljSePomerio = true; else crniKraljSePomerio = true; }
                    if (figura == "R")
                    {
                    ...
                    if (!imaPoteza)
                    {
                        ...
                        {
                            // POPRAVLJENO TAKOĐE: Pat pozicija automatski donosi po 0.5 poena obojici
                            resultadoBeli += 0.5;
                            rezultatCrni += 0.5;
                            OsveziNaslovProzora();
        ...
        void ObeleziMogucePoteze(int pocetnoPolje, string figura, bool bojaIgraca)
        {
            ...
            for (int i = 0; i < 64; i++)
            {
                ...
                if (isPossible(pocetnoPolje, i, figura, bojaIgraca))
                {
                    ...
                    bool jeEP = (figura == "P" && Math.Abs(pKol - ciljKol) == 1 && string.IsNullOrEmpty(staraCiljFigura));
        ...
        private void ResetujSelekciju()
        {
            ...
            for (int row = 0; row < 8; row++)
                for (int col = 0; col < 8; col++)
                    board[row, col].BackColor = ((row + col) % 2 == 0) ? Color.Beige : Color.Brown;
        }
        ...
        bool IsKingInCheck(string bojaKralja)
        {
            ...
            for (int i = 0; i < 64; i++)
            {
                ...
                if (!string.IsNullOrEmpty(boardf[r, c]) && boardc[r, c] != bojaKralja)
                {
                    ...
                    {
                        if ("RBQ".Contains(protivnickaFigura)) { if (Put(i, kraljIndeks)) return true; }
                        else if (protivnickaFigura == "N") return true;
                        else if (protivnickaFigura == "P" && Math.Abs(c - (kraljIndeks % 8)) == 1) return true;
                    }
                }
            }
            return false;
        ...
        bool IgracImaLegalnihPoteza(string bojaIgraca)
        {
            ...
            for (int i = 0; i < 64; i++)
            {
                ...
                {
                    string fig = boardf[pRed, pKol];
                    for (int j = 0; j < 64; j++)
                    {
                        int kRed = j / 8; int kKol = j % 8;
                        bool validan = isPossible(i, j, fig, jeBeli);
                        if (validan && "RBQ".Contains(fig)) validan = Put(i, j);

                        if (validan && boardc[kRed, kKol] != bojaIgraca)
                        {
                            string staraCF = boardf[kRed, kKol]; string staraCB = boardc[kRed, kKol];
                            bool jeEP = (fig == "P" && Math.Abs(pKol - kKol) == 1 && string.IsNullOrEmpty(staraCF));
                            string epF = null; string epB = null;
